...
fun <T : AttachedMedia<T>> List<T>.markToRemove(item: T): List<T> =
    toMutableList().apply { this[indexOfFirst { it.id == item.id }] = item.markToRemove() }

/** Ext fun that creates patch for media (new local non removed media and removed remote media) */
fun <T : AttachedMedia<T>> List<T>.formPatch(): List<T> = filter {
    (it.local && !it.toRemove) || (!it.local && it.toRemove)
}

/** Ext fun that updates item in list, item must implement [Id] */
fun <T : Id> List<T>.updateById(entity: Id, reduced: (T) -> T): List<T> =
    toMutableList().apply {
