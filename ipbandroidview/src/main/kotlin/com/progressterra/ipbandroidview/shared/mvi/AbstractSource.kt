 ...
 * @property loadPage - function to load page, where first in result pair is count of raw source
 *   items and second is list of items
 */
abstract class AbstractSource<I, O : Any> : PagingSource<Int, O>() {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, O> {
        ...
        val response =
            loadPage(skip = (nextPage - 1) * pageSize, take = pageSize).onSuccess {
