...
abstract class AbstractLoggingUseCase(
    ...
    protected suspend fun <T> handle(block: suspend () -> T): Result<T> =
        runCatching { block() }
            .onFailure {
                when (it) {
                    is UnknownHostException,
                    is SocketTimeoutException,
                    is ConnectException -> makeToastUseCase(R.string.no_internet_connection)
                    is ToastedException ->
                        if (it.stringId == 0) makeToastUseCase(it.customMsg!!)
                        else makeToastUseCase(it.stringId)
