...
fun DHCheckPerformedFullDataViewModel.toChecklistDocument() =
    ChecklistDocument(
        ...
        stats =
            ChecklistStats(
                ...
                remaining =
                    tryOrNull { countDR!! - countDRPositiveAnswer!! - countDRNegativeAnswer!! } ?: 0
            )
    )
...
fun List<Check>.createStats(): ChecklistStats {
    var successful = 0
    var failed = 0
    this.forEach { if (it.yesNo == true) successful++ else if (it.yesNo == false) failed++ }
    val total = this.size
...
@Composable
fun TypeStatusOrder.toString(stringResource: @Composable (Int) -> String) =
    when (this) {
...
fun SuggestionExtendedInfo.convertSuggestionToAddressUIModel(time: String) =
    Address(
...
fun CharacteristicData.toDocument(gson: Gson, createId: CreateId) =
    Document(
        ...
        entries =
            (characteristicValue?.valueAsJSON?.let {
                    ...
                    TextFieldState(
                            ...
                            type =
                                when (it.typeValue) {
                                    ...
                                    TypeValueCharacteristic.AS_CUSTOM_AS_JSON ->
        ...
        photo =
            DocumentPhotoState(
                items =
                    characteristicValue?.listImages?.map { img ->
                        MultisizedImage(
                            id = img.idUnique!!,
                            local = false,
                            toRemove = false,
...
fun RFCharacteristicValueViewModel.toDocument(gson: Gson, createId: CreateId) =
    Document(
        status = statusDoc ?: TypeStatusDoc.NOT_FILL,
        name = characteristicType?.name ?: "",
        id = idUnique!!,
        entries =
            (valueAsJSON?.let {
...
fun DRSaleForCartAndOrder.toReceiptItems() =
    ReceiptState.Item(
        name = nameGoods ?: "",
        quantity = quantity ?: 0,
...
fun RGMessagesViewModel.toMessage() =
    Message(
        id = idUnique!!,
...
fun RGDialogsViewModel.toDatingChat() = DatingChat(id = idUnique!!, name = description ?: "")
...
@Composable
fun TypeStatusDoc.toString(stringResource: @Composable (Int) -> String) =
    when (this) {
...
fun TypeStatusDoc.toCanBeEditted() =
    when (this) {
...
fun RFPaymentDataForClientViewModel.toBankCardState() =
    BankCardState(
        id = idUnique!!,
        document =
            Document(
...
fun DHPaymentClientViewModel.toWithdrawalTransactionState() =
...
@Composable
fun TypeResultOperationBisinessArea.toColor() =
    when (this) {
...
fun TypeResultOperationBisinessArea.toString(stringResource: @Composable (Int) -> String) =
    when (this) {
        TypeResultOperationBisinessArea.IN_PROGRESS ->
            stringResource(R.string.transaction_in_progress)
...
fun Connection.toString(stringResource: @Composable (Int) -> String) =
    if (isEmpty()) stringResource(R.string.send_request)
    else
        when (type) {
            EnumTypeStatusConnect.WAIT ->
                if (own) stringResource(R.string.request_sent)
                else stringResource(R.string.accept_connect)
            EnumTypeStatusConnect.SUCCESS -> stringResource(R.string.connect_accepted)
            EnumTypeStatusConnect.REJECT -> ""
        }

fun RFInterestViewModel.toInterest() = Interest(id = idUnique!!, name = name ?: "")

fun RGClientDataViewModel.toDatingUser(own: Boolean = false) =
    DatingUser(
        id = idClient!!,
        name = this.nickName ?: "",
...
fun RGClientDataViewModelWithDistance.toDatingUser() =
    DatingUser(
        id = idClient!!,
        name = this.nickName ?: "",
        description = descriptionAboutMe ?: "",
...
fun com.progressterra.ipbandroidapi.api.scrm.models.TypeSex.toSex() =
    when (this) {
...
fun RFTargetViewModel.toDatingTarget() = DatingTarget(id = idUnique!!, name = name ?: "")

fun RGConnectViewModel.toDatingUser(own: Boolean) =
    (if (own) targetClientData!! else initiatorClientData!!)
        .toDatingUser()
        .copy(
            connection =
                Connection(
