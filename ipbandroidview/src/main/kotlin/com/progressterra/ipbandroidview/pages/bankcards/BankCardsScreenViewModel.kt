...
class BankCardsScreenViewModel(
...
) : AbstractNonInputViewModel<BankCardsScreenState, BankCardsScreenEffect>(), UseBankCardsScreen {
    ...
    override fun refresh() {
        onBackground {
            ...
            var isSuccess = true
            fetchUnconfirmedBankCardsUseCase()
                .onSuccess { cards -> emitState { it.copy(otherCards = cachePaging(cards)) } }
                .onFailure { isSuccess = false }
            fetchConfirmedBankCardsUseCase()
                .onSuccess { cards -> emitState { it.copy(addedCards = cachePaging(cards)) } }
                .onFailure { isSuccess = false }
    ...
    override fun handleEvent(event: BankCardEvent) {
        when (event) {
            is BankCardEvent.Click ->
                postEffect(BankCardsScreenEffect.OpenDetails(event.state.document))
