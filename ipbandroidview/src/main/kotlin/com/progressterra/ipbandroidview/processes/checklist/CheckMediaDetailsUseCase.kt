...
interface CheckMediaDetailsUseCase {
    ...
    class Base(
        ...
        override suspend fun invoke(check: Check): Result<CurrentCheckMedia> = withToken { token ->
            val voices = mutableListOf<Voice>()
            val pictures = mutableListOf<MultisizedImage>()
            mediaDataService
                .attachedToEntity(token, check.id, FilterAndSort(emptyList(), null, "", 0, 300))
                .dataList
                ?.forEach { item ->
                    if (item.contentType == TypeContent.IMAGE) {
                        ...
                        saveAudio(
                            mediaDataService.downloadFile(token, item.urlData!!).byteStream(),
                            item.idUnique!!
                        )
                        voices.add(Voice(id = item.idUnique!!, local = false, toRemove = false))
                    }
                }
            CurrentCheckMedia(voices = voices, pictures = pictures)
        }
