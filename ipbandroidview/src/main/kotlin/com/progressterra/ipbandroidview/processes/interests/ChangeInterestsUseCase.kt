...
interface ChangeInterestsUseCase {
    ...
    class Base(
        ...
        override suspend fun invoke(user: List<Interest>, changed: List<Interest>): Result<Unit> =
            withToken { token ->
                changed.forEach {
                    if (user.contains(it)) {
                        if (
                            service
                                .clientInterestDelete(
                                    token = token,
                        ...
                        if (
                            service
                                .clientInterest(token = token, body = IncomeDataIDRFInterest(it.id))
                                .result
                                ?.status != StatusResult.SUCCESS
                        )
                            throw ToastedException(R.string.failure)
